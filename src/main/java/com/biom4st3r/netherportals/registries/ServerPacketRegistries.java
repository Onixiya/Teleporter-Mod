...
public class ServerPacketRegistries
{
	...
	{
		if(false)
			System.out.println(NetherPortalMod.MODID_SHORT + ": " + message);
	}
    ...
    public static void reg()
    {
		...
		ServerSidePacketRegistry.INSTANCE.register(Packets.REGISTER_NEW_PORTAL, (context, buffer) ->
		{
			...
			boolean is_bird_bath = context.getPlayer().getEntityWorld().getBlockState(bpos).getBlock() instanceof BirdBathBlock;
			//String uuid = packetContext.getPlayer().getUuidAsString();
			if(within_range && is_bird_bath)
			{
			...
			else
			{
				if(!within_range)
				{
					System.out.println(context.getPlayer().getEntityName() + ": Attempted to register a new Ender Bird Bath > 10 Blocks away from themselfs. Vanilla Reach is ~5");
				}
				if(!is_bird_bath)
				{
					System.out.println(context.getPlayer().getEntityName() + ": Attempted to register a new Ender Bird Bath at location that's not a portal");
				}
			}
		});
		...
		ServerSidePacketRegistry.INSTANCE.register(Packets.REQUEST_TELEPORT, (packetContext, packetByteBuf) ->
		{
			...
			if(dt != pe.dimension)
			{
				NetherPortalMod.interDimesionalTeleport(pe, dt, blockPos.add(0, 1, 0));
			}
			else
			{
			...
			//checking if valid
			BlockState birdbath = pe.world.getBlockState(blockPos);
			pe.getEntityWorld().sendEntityStatus(pe, (byte)46);
			if(birdbath.getBlock() instanceof BirdBathBlock) // birdbath.isAir())
			{
		...
		ServerSidePacketRegistry.INSTANCE.register(Packets.SEARCH_PLAYER, (packetContext,packetByteBuf) ->
		{
			...
			if(pl.names.size() > 0 && !searchingForSelf)
			{
